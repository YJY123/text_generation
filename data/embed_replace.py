...
# -*- coding: utf-8 -*-
'''
@Description: 文本增强 ---> 利用相似词替换文本中的非关键词
@FilePath: /text_generation/data/embed_replace.py
'''
...
from model.config import root_path
...
class EmbedReplace():
    def __init__(self, sample_path, wv_path):
        ...
        if os.path.exists(os.path.join(root_path, 'saved/tfidf.model')):
            self.tfidf_model = TfidfModel.load(os.path.join(root_path, 'saved/tfidf.model'))
            self.dct = Dictionary.load(os.path.join(root_path, 'saved/tfidf.dict'))
            self.corpus = [self.dct.doc2bow(doc) for doc in self.refs]
        else:
            self.dct = Dictionary(self.refs)
            self.corpus = [self.dct.doc2bow(doc) for doc in self.refs]
            self.tfidf_model = TfidfModel(self.corpus)
            self.dct.save(os.path.join(root_path, 'saved/tfidf.dict'))
            self.tfidf_model.save(os.path.join(root_path, 'saved/tfidf.model'))
...
if __name__ == '__main__':
    sample_path = os.path.join(root_path, 'output/train.txt')
    wv_path = os.path.join(root_path, 'word_vectors/merge_sgns_bigram_char300.txt')
    replacer = EmbedReplace(sample_path, wv_path)
    replacer.generate_samples(os.path.join(root_path, 'output/replaced.txt'))
